...
// Converts from a soda-format object to a REST-format object.
// Does not strip out anything based on a lack of authentication.
const oracleObjectToParseObject = (className, oracleObject, schema) => {
  log.verbose('entered oracleObjectToParseObject');
  log.verbose('className = ' + className);
  log.verbose('oracleObject = ' + JSON.stringify(oracleObject));
  log.verbose('typeof oracleObject = ' + typeof oracleObject);
  log.verbose('schema = ' + JSON.stringify(schema));
  switch (typeof oracleObject) {
    case 'string':
    case 'number':
    case 'boolean':
    ...
    case 'symbol':
    case 'function':
      throw 'bad value in oracleObjectToParseObject';
    case 'object': {
      if (oracleObject === null) {
        return null;
      }
      ...
      // if (oracleObject instanceof Long) {
      //   return oracleObject.toNumber();
      // }

      // if (oracleObject instanceof Double) {
      //   return oracleObject.value;
      // }
      ...
      const restObject = {};
      if (oracleObject._rperm || oracleObject._wperm) {
        restObject._rperm = oracleObject._rperm || [];
        restObject._wperm = oracleObject._wperm || [];
        delete oracleObject._rperm;
        delete oracleObject._wperm;
      }

      for (var key in oracleObject) {
        switch (key) {
          case '_id':
            restObject['objectId'] = '' + oracleObject[key];
            break;
          case '_hashed_password':
            restObject._hashed_password = oracleObject[key];
            break;
          case '_acl':
            break;
          case '_email_verify_token':
          ...
          case '_password_history':
            ...
            restObject[key] = oracleObject[key];
            break;
          case '_session_token':
            restObject['sessionToken'] = oracleObject[key];
            break;
          case '_sodaDocumentMarker':
            restObject['sodaDocumentMarker'] = oracleObject[key];
            break;
          case 'updatedAt':
          case '_updated_at':
            restObject['updatedAt'] = DateCoder.DatabaseToJSON(oracleObject[key]);
            if (restObject['updatedAt'] !== null)
              restObject['updatedAt'] = Parse._encode(restObject['updatedAt']).iso;
            break;
          case 'createdAt':
          case '_created_at':
            restObject['createdAt'] = DateCoder.DatabaseToJSON(oracleObject[key]);
            if (restObject['createdAt'] !== null)
              restObject['createdAt'] = Parse._encode(restObject['createdAt']).iso;
            break;
          case 'expiresAt':
          case '_expiresAt':
            restObject['expiresAt'] = DateCoder.DatabaseToJSON(oracleObject[key]);
            if (restObject['expiresAt'] !== null)
              restObject['expiresAt'] = Parse._encode(restObject['expiresAt']);
            break;
          case 'lastUsed':
          case '_last_used':
            restObject['lastUsed'] = DateCoder.DatabaseToJSON(oracleObject[key]);
            if (restObject['lastUsed'] !== null)
              restObject['lastUsed'] = Parse._encode(restObject['lastUsed']).iso;
            break;
          case 'timesUsed':
          case 'times_used':
            restObject['timesUsed'] = oracleObject[key];
            break;
          case 'authData':
            if (className === '_User') {
              log.warn(
              ...
              restObject['authData'] = oracleObject[key];
            }
            break;
          default:
            ...
            if (authDataMatch && className === '_User') {
              ...
              restObject['authData'][provider] = oracleObject[key];
              break;
            }
            ...
            if (key.indexOf('_p_') == 0) {
              ...
              if (oracleObject[key] === null) {
                break;
              }
              restObject[newKey] = transformPointerString(schema, newKey, oracleObject[key]);
              break;
            } else if (key[0] == '_' && key != '__type') {
              throw 'bad key in untransform: ' + key;
            } else {
              var value = oracleObject[key];
              if (
                schema.fields[key] &&
              ...
              if (
                schema.fields[key] &&
                schema.fields[key].type === 'Date' 
              ) {
                restObject[key] = Parse._encode(DateCoder.DatabaseToJSON(value));
                break;
              }
            }
            restObject[key] = nestedOracleObjectToNestedParseObject(oracleObject[key]);
        }
      }
...
var BytesCoder = {
  ...
  databaseToJSON(object) {
    let value;
    if (this.isBase64Value(object.bytes)) {
      value = Buffer.from(object.bytes, 'base64').toString('ascii');
    } else {
      value = object.bytes;
    }
    return {
  ...
  isValidDatabaseObject(object) {
    return this.isBase64Value(object.bytes);
  },

  JSONToDatabase(json) {
    return Buffer.from(json.base64).toString('base64');
  },
...
var GeoPointCoder = {
  databaseToJSON(object) {
    ...
    for (const x in object) {
      if (x === 'coordinates' && object[x].length == 2) {
        long = object[x][0];
        lat = object[x][1];
      }
    }
  ...
  isValidDatabaseObject(object) {
    ...
    return numfields == 2 && foundfields == 2;
  },
